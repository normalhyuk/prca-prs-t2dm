...
# STEP 2: White-British ancestry
# Reference: UKBB Genetic PC documentation + pan-UKBB reference panel
# https://pan.ukbb.broadinstitute.org/docs/qc

pheno <- pheno %>% 
  # Join UKBB-provided genetic data
  left_join(pca %>% select(eid, 
                          PC1 = genetic_principal_components.f22009_0_0,
                          PC2 = genetic_principal_components.f22010_0_0,
                          ethnicity_self = genetic_ethnic_background.f22006_0_0),
            by = "eid") %>%
  
  # UKBB OFFICIAL White-British criteria (Showcase documentation):
  # 1. Self-reported "British" ancestry (Field 22006 == 1001)
  # 2. PCA cluster within pan-UKBB reference bounds
  filter(ethnicity_self == 1001) %>%  # British only (not "White" or "Irish")
  
  # Pan-UKBB reference panel cluster bounds (UKBB standard values)
  filter(PC1 >= -0.0180 & PC1 <= 0.0135) %>%  # Official WB bounds
  filter(PC2 >= -0.0250 & PC2 <= 0.0400)       # Secondary confirmation

cat("White-British N:", nrow(pheno), "\n")
